<x-admin-layout title="Detail Pengajuan Tukar Buku">
    <!-- Back Button -->
    <div
        class="mb-6                        @foreach ($exchange->photos as $photo)
                            <div class="relative group">
                                <img src="{{ asset('storage/' . $photo->photo_path) }}" 
                                     alt="Book photo" 
                                     class="w-full h-48 object-cover rounded-lg cursor-pointer hover:opacity-75 transition-opacity"
                                     onclick="openImageModal('{{ asset('storage/' . $photo->photo_path) }}')">      <a href="{{ route('admin.book-exchanges') }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($exchange->photos as $photo)
                            <div class="relative group">
                                ...
                                <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-25 rounded-lg transition-opacity flex items-center justify-center">
...
<p class="text-gray-500">
        Tidak ada foto yang diunggah.</p>
        @endif
    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            ...
            <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold text-brand-dark">{{ $exchange->user->bookExchanges->count() }}</span>
                </div>
                <div class="flex justify-between">
